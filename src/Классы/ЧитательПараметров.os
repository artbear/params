...
Процедура ПриСозданииОбъекта(Знач НовыйПутьФайлаНастроек = "")
	Если Не ЗначениеЗаполнено(НовыйПутьФайлаНастроек) Тогда
		...
		ПутьФайлаНастроекПоУмолчанию = ОбъединитьПути(ТекущийКаталог(), ПутьФайлаНастроекПоУмолчанию);
		ФайлПоУмолчанию = Новый Файл(ПутьФайлаНастроекПоУмолчанию);
...
//
// Параметры:
//   ПутьФайлаНастроек - <Строка> - путь файла
//
Процедура УстановитьФайлПоУмолчанию(Знач НовыйПутьФайлаНастроек) Экспорт
	...
	ПутьФайлаНастроек = НовыйПутьФайлаНастроек;
КонецПроцедуры
...
Процедура ДобавитьПараметрВКоллекциюСоответствияПеременныхОкружения(РезСоответствие, Параметры, ПрефиксПеременныхСреды)
	Для каждого Параметр Из Параметры Цикл
		...
		ИмяПеременнойСреды = СтрШаблон("%1_%2", 
...
Процедура ДополнитьЗначенияПараметров(Знач ЗначенияПараметров, Знач Команда, Знач КлючФайлаНастроек, 
	...
	КонецЕсли;
	НастройкиИзФайла = ПрочитатьНастройкиФайлJSON(ТекущийКаталогПроекта, ПутьКФайлуНастройки);
...
Функция ПрочитатьНастройкиФайлJSON(Знач ТекущийКаталогПроекта, Знач ПутьКФайлу )
	Рез = Новый Соответствие;
