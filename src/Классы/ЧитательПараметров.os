...
Процедура ОбработатьШаблонныеПодстановки(Параметры)
	...
	Для Счетчик = 1 По 5 Цикл
		...
		СоответствиеПодстановок = Новый Соответствие;
		
		НайтиШаблонныеПодстановки(Параметры, СоответствиеПодстановок, РегулярноеВыражение);

		Если НЕ ЗначениеЗаполнено(СоответствиеПодстановок) Тогда
			Прервать;			
		КонецЕсли;
		...
		ВыполнитьПодстановки(СоответствиеПодстановок, ЗначенияКлючей);

	КонецЦикла;

КонецПроцедуры

Процедура НайтиШаблонныеПодстановки(Параметры, Знач СоответствиеПодстановок, Знач РегулярноеВыражение)
	
	Для каждого КлючЗначение Из Параметры Цикл
		Ключ = КлючЗначение.Ключ;
		Значение = КлючЗначение.Значение;
		Описание = Новый Структура("Ключ, Значение, ИспользоватьКлюч, Параметры", Ключ, Значение, Ложь, Параметры);
		Описание.Вставить("КоллекцияСовпадений", Неопределено);
		...
		Если ТипЗнч(Значение) = Тип("Строка") Тогда
			...
			Если КоллекцияСовпадений.Количество() > 0 Тогда
				...
				СоответствиеПодстановок.Вставить(Значение, Описание);
		...
		Если ТипЗнч(Ключ) = Тип("Строка") Тогда
			КоллекцияСовпадений = РегулярноеВыражение.НайтиСовпадения( Ключ );
			Если КоллекцияСовпадений.Количество() > 0 Тогда
				Описание.ИспользоватьКлюч = Истина;
				Описание.Вставить("КоллекцияСовпадений", КоллекцияСовпадений);
				СоответствиеПодстановок.Вставить(Ключ, Описание);
				
				Лог.Отладка("Нашли ключ для подстановки <%1>", Ключ);
			КонецЕсли;
		КонецЕсли;

		Если ТипЗнч(Значение) = Тип("Соответствие") Тогда
			НайтиШаблонныеПодстановки(Значение, СоответствиеПодстановок, РегулярноеВыражение);
		КонецЕсли;
...
Процедура ВыполнитьПодстановки(СоответствиеПодстановок, ЗначенияКлючей)
	Для каждого КлючЗначение Из СоответствиеПодстановок Цикл
		ОписаниеПодстановки = КлючЗначение.Значение;
		КлючПодстановки = ОписаниеПодстановки.Ключ;
		ЗначениеПодстановки = ОписаниеПодстановки.Значение;

		КоллекцияСовпадений = ОписаниеПодстановки.КоллекцияСовпадений;
		Если ЗначениеЗаполнено(КоллекцияСовпадений) Тогда
			Для каждого Совпадение Из КоллекцияСовпадений Цикл
				...
				Если ЗначениеПоКлючу <> Неопределено Тогда
					НовыйКлюч = КлючПодстановки;
					НовоеЗначение = ЗначениеПодстановки;
					Если ОписаниеПодстановки.ИспользоватьКлюч Тогда
						ОписаниеПодстановки.Параметры.Удалить(КлючПодстановки);
						НовыйКлюч = ЗначениеПоКлючу;
					Иначе
						НовоеЗначение = ЗначениеПоКлючу;
					КонецЕсли;
					ОписаниеПодстановки.Параметры.Вставить(НовыйКлюч, НовоеЗначение);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
